Model tp091r
  ...
  Variables
    ...
    x[4] = -1/2 ifelse(stricths,1,`',`, >= 1/10') ! not a bug
    x[5] =  1/2 ifelse(stricths,1,`',`, >= 0')
    obj
  End Variables
  ...
  Intermediates
    ...
    rhoaux[5][1:30] = 1*emx[1][1:30]*emx[2][1:30]*emx[3][1:30] &
                                    *emx[4][1:30]*emx[5][1:30] &
                    - 2*emx[2][1:30]*emx[3][1:30]*emx[4][1:30] &
                                                 *emx[5][1:30] &
                    + 2*emx[3][1:30]*emx[4][1:30]*emx[5][1:30] &
                    - 2*emx[4][1:30]*emx[5][1:30] &
                    + 2*emx[5][1:30] &
                    - 1
    rho[1:30] = (-1)*rhoaux[5][1:30]/mu[1:30]^2
    aux1[1:30] = A[1:30]^2*rhoaux[5][1:30]^2
    aux3[1:30] = A[1:30]*rho[1:30]
    hsum[1:30] = hsum[0:29] + aux1[1:30]*aux2[1:30] &
                            + aux3[1:30]*aux4[1:30]
    h = hsum[30]
    mfcorrr = (-1)*x[2] + (x[3] - 8/10)^2 + x[5]
  End Intermediates
  ...
  Equations
    ...
    obj = x[1]^2 + x[2]^2 + x[3]^2 + x[4]^2 + x[5]^2 &
    ...
    ! x[1] = 0.717050023543273
    ! x[2] = 0
    ! x[3] = 0.8
    ! x[4] = 0.456613708320804
    ! x[5] = 0
