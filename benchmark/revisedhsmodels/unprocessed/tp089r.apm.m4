Model tp089r
  ...
  Variables
    ...
    x[2] = -1/2 ifelse(stricths,1,`',`, <= -1/10')
    x[3] =  1/2 ifelse(stricths,1,`',`, >=  0')
    obj
  End Variables
  ...
  Intermediates
    ...
    rhoaux[3][1:30] = 1*emx[1][1:30]*emx[2][1:30]*emx[3][1:30] &
                    - 2*emx[2][1:30]*emx[3][1:30] &
                    + 2*emx[3][1:30] &
                    - 1
    rho[1:30] = (-1)*rhoaux[3][1:30]/mu[1:30]^2
    aux1[1:30] = A[1:30]^2*rhoaux[3][1:30]^2
  ...
  Equations
    ...
    ! x[1] =  1.07431873122617
    ! x[2] = -0.456613708320804
    ! x[3] =  0
